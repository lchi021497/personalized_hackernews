...
def parse_neighbors(
    ...
    while sibling is not None and num_count <= rolling_window_len:
        ...
        if is_paragraph(text, pgraph_len_thres):
            num_count = 0
        elif list_as_paragraph and (
            sibling.name == "ol" or sibling.name == "ul" or sibling.name == "li"
        ):
            num_count = 0
...
def parse_paragraphs(
    pgraphs, pgraph_len_thres, rolling_window_len, list_as_paragraph=False
):
    ...
    while i < len(pgraphs):
        ...
        if is_paragraph(p.text, pgraph_len_thres):
            ...
            local_parsed, end_index = parse_neighbors(
                i,
                pgraphs,
                pgraph_len_thres,
                rolling_window_len,
